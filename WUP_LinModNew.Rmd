...
summary.lm(model.3)
```
This data gives an equation of SIMS= 0.037311 x ARM + 0.024470 x GRIP -5.4338771 for what would be the plane of best fit. 

#### predict model.3 at target point
```{r}
predict(model.3, tgtpoint, interval = c("prediction"))
```
The table above gives us a value of 0.149647 as the target point for SIMS vs ARM+GRP. With a 95% interval the value will be between -2.132373 and 2.431668.
...
```
From the data table, we are able to see that in this case model.3 was more accurate than model.2. If there were to be no differences between the two models, we would have 0.000000001495% chance of getting these exact results. The difference in degrees of freedom is 1 since model.2 had 145 and model.3 had 144. The residual sum of squares for each model is 243.07 for model.2 and 188.43 for model.3. This gives us a difference of 54.639 errors making the model.3 more accurate than the model.2.

## Informally compare Model.1 with model.2
```{r}
anova(model.1,model.2)
```
From this data model.1 has a residual sum of squares of 217.88 and model.2 has 243.07. This gives us a difference of -25.189 which shows us that the model.1 is a more accurate model when comparing SIMS to ARM and GRIP individually.

#Conclusion
In conclusion, model.3 was more accurate than model.2 and model.3 was more accurate than model.1. Model.1 is a more accurate model when comparing SIMS to ARM and GRIP individually. The SIMS vs ARM model is more accurate than the SIMS vs GRIP model. 